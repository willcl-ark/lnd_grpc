...
from lib import rpc_pb2_grpc as pb2_grpc
import inspect
...
class Attributes:
    ...
    def __init__(self):
        ...
        self.lnd_grpc_list = [o for o in inspect.getmembers(py_rpc.Client) if inspect.isfunction(o[1])]
        self.lnd_grpc_names = []
        for func in self.lnd_grpc_list:
            if func[0] != "__init__":
                self.lnd_grpc_names.append(''.join(x.capitalize() or '_' for x in func[0].split('_')))


        # Get all functions from LightningServicer into a list
        self.lightning_servicer_list = [o for o in inspect.getmembers(pb2_grpc.LightningServicer) if inspect.isfunction(o[1])]
        ...
        self.wallet_unlocker_list = [o for o in inspect.getmembers(pb2_grpc.WalletUnlockerServicer) if inspect.isfunction(o[1])]
